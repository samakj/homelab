...
void onDS18B20TemperatureChange(float temperature)
{
  if(temperature != Homelab::Sensors::DS18B20::TEMPERATURE_NULL_VALUE)
    Homelab::Server::sendReport(temperature, "temperature", DS18B20_TEMPERATURE_TAGS);
  else Homelab::Server::sendReport(nullptr, "temperature", DS18B20_TEMPERATURE_TAGS);
}
