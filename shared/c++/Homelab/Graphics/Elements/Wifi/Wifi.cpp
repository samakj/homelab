...
Homelab::Graphics::Elements::Wifi::Wifi(
    ...
    : Text(
          x, y, height, width, fill, background, "No Wifi", font, colour, padding, verticalDatum,
          horizontalDatum, zIndex
      ) {};

void Homelab::Graphics::Elements::Wifi::loop(TFT_eSPI* tft)
{
  if(Homelab::Wifi::isConnecting()) Homelab::Graphics::Elements::Text::setText("Connecting");
  else if(Homelab::Wifi::isConnected())
    Homelab::Graphics::Elements::Text::setText(Homelab::Wifi::getSSID());
  else Homelab::Graphics::Elements::Text::setText("No Wifi");
